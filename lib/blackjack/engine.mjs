/** @typedef {'deal'|'hit'|'stand'} Action */
/** @typedef {'ready'|'player'|'between'|'lost'|'cleared'} Phase */
/** @typedef {{ player:number[], dealer:number[], next:number, hidden:boolean, phase:Phase, round:number, handsWon:number, message:string }} State */
...
/** @param {number} card */
export function cardName(card) { return RANKS[rank(card)] + ' of ' + SUITS[Math.floor(card / 13)]; }
/** @param {number[]} cards */
export function total(cards) {
  let value = 0, aces = 0;
  for (const card of cards) { const r = rank(card); value += r === 0 ? 11 : Math.min(r + 1, 10); if (r === 0) aces++; }
...
/** @param {State} state */
export function points(state) { return state.handsWon * 100 + score(state); }
/** @param {State} state */
...
export function step(state, action, deck, rulesVersion = RULES_VERSION) {
  if (!actions(state).includes(action)) throw new Error('That action is not available.');
