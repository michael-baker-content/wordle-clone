...
import { actions, replay, type Action } from "../../../lib/blackjack/engine.mjs";
...
export async function POST(request: Request) {
  try {
    ...
    if (!actions(state).includes(body.action)) return error("That action is no longer available.", 400);
